...
build_ievm() {
    ...
    then
        ...
        then
            ...
            get_md5=$(node -pe "JSON.parse(process.argv[1])["${list_version}"].software[0].files[1].md5.slice(0, 31).concat('vms', JSON.parse(process.argv[1])[5].software[0].files[0].md5.slice(34,).toString())" "$(curl -s https://developer.microsoft.com/en-us/microsoft-edge/api/tools/vms/)")
            log "Grabbing md5 file to parse for md5 string"
            md5=$(curl "${get_md5}" | tr '[:upper:]' '[:lower:]')
        fi
